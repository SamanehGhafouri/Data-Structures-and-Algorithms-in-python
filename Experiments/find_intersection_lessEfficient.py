def common_element(list_a, list_b):
    common = set()
    while len(list_a) != 0:
        common.add(list_a.pop())

    i = 0
    while i < len(list_b):
        if list_b[i] in common:
            i += 1
        else:
            list_b.pop(i)       # time complexity depends on python how to pop
    common = set(list_b)        # in worst case O(n^2) in best case O(n)
    return common


li_a = [2, 1, 2, 3]
li_b = [2, 3, 5, 2]
print(common_element(li_a, li_b))
